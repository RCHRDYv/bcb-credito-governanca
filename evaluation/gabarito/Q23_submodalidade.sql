-- PT: Q23, leitura por submodalidade. Crescimento da carteira e do volume de PIX nas
--     duas janelas, 12 meses e o recorte inteiro, com a marca de retração.
--     Coincidência no tempo não é causa.
-- EN: Q23, reading by submodality. Portfolio and PIX growth over 12 months and over the
--     whole window, flagging contraction. Coincidence is not causation.
with carteira as (

    select f.data_base, m.codigo_submodalidade as codigo, m.submodalidade as nome, cast(sum(f.carteira_ativa) as double) as carteira_ativa
    from fct_carteira f
    join dim_modalidade m
        on m.codigo_submodalidade = f.codigo_submodalidade
    group by f.data_base, m.codigo_submodalidade, m.submodalidade
